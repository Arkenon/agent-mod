...
### ❌ Eksik Alanlar & Riskler (Notlar Işığında Çıkarılanlar)
- **Veri Modeli Yokluğu:** `agentmod_agent` ve `agentmod_conversation` veri yapıları henüz veritabanına basılmıyor, kalıcı hafıza (history persistence) bulunmuyor.
- **Yapısal Veri Gösterimi Eksikliği (DataViews):** Ajan "Son 3 yazıyı bul" dediğinde sonuçlar ham markdown metin olarak dökülüyor. WordPress'in yeni nesil interaktif tablo bileşeni olan *DataViews* entegrasyonu bulunmuyor.
...
- **Gutenberg Bağlantısı Kopukluğu:** Blok editörün içinden AI Workspace'e bağlamsal geçiş (handoff) senaryosu tasarlanmamış.
- **Ajan ve Model Yönetim UI Eksikliği:** Kullanıcı arayüzden ajan seçemiyor, statik bir bot çalışıyor.
...
``` plaintext
[Kullanıcı Mesajı / Dosya] 
       │
       ▼
[ChatPanel / Composer] ──► Context Scope Kontrolü (Site Context / General Knowledge)
       │
       ▼
[ConversationManager] ──► Oturum ve Token Limit Kontrolü (Hard Limits)
       │
       ▼
[HistoryManager] ──► Geçmiş Mesajların Sıkıştırılması / Özetlenmesi
       │
       ▼
[PromptBuilder] ──► System Prompt + Kişilik + RAG / Site Bağlamı Birleştirme
       │
       ▼
[AbilityResolver] ──► WP Ability Registry Filtreleme + Yetki/Capability Kontrolü
       │
       ▼
[AIClientAdapter] ──► (DÖNGÜ) WP AI Client ◄──► Canlı LLM Sağlayıcısı
       │
       ...
       │
       ▼
[Artifact Sınıflandırma] ──► Markdown Metin / DataViews Tablo / Code Snippet
       │
       ▼
[ChatPanel / MessageItem] ──► Eylem Butonları ile Render (Copy, Create Draft, Edit Link)
```
...
*   **Yetenek Yetki Kontrolü (Middleware Layer):** AI motoru veritabanına doğrudan erişemez. Her yetenek çağrısından önce PHP katmanında `current_user_can()` kontrolü yapılır. Örneğin; `edit_private_posts` yetkisi olmayan bir kullanıcının seansı için ajan gizli postları listeleyemez ve *"Yetkiniz yetersiz"* hatası fırlatır.
...
*   **Meta Alanları:**
    ...
    *   `agent_type` (Select): `generic`, `visitor`, `lead`, `content`, `support`.
    *   `system_prompt` (WYSIWYG / Editor Override).
    *   `role` (Select) & `goal` (Textarea).
    *   `personality_traits` (Multi-select): `helpful`, `friendly`, `professional`, `corporate`, `persuasive`, `curious`, `patient`, `analytical` *(PromptBuilder'da kullanılacak)*.
    *   `allowed_abilities` (Multi-select): Core Registry'den okunan ve bu ajana izin verilen yetenekler (`all`, `selected`).

### 2. `agentmod_conversation` (Sohbet Oturumları CPT)
*   **Meta Alanları:**
    *   `agent_id` (Relationship): Bağlı olduğu ajan.
    ...
    *   `messages_history` (JSON / LongText): `{role: 'user'|'assistant'|'tool', content: '...', tool_calls: []}` dizisi.

---

## 5. İleri Düzey Arayüz (UX) ve Çıktı Bileşenleri
...
### ⚙️ Phase 2.0 — Orta Vade (Veri Modeli, Hafıza ve Güvenlik Arayüzleri)
...
### 🚀 Phase 3.0 — Uzun Vade (DataViews, Gutenberg Handoff ve Tam Ekran Deneyimi)
...
## 7. MVP Kapsam Dışı Bırakılanlar (De-Scoped)
